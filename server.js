    ...
    socket.on('place_secure_bet', async (data, callback) => {
        try {
            ...
            if (typeof callback === 'function') {
                callback({ success: true, newBalance: finalBal });
            }
        } catch (err) {
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; outline: none; -webkit-tap-highlight-color: transparent; }
        ...
        .bg-wheel-wrap { position: relative; width: 320px; height: 320px; border-radius: 50%; border: 10px solid #ffd700; box-shadow: 0 0 100px rgba(255, 215, 0, 0.8); animation: wheelRotateBg 35s linear infinite; display: flex; align-items: center; justify-content: center; }
        #loginWheelCanvasBg { width: 320px; height: 320px; border-radius: 50%; display: block; filter: brightness(1.3); }
        ...
        #loginScreenContainer { 
            ...
            padding: 10px; position: absolute; top: 0; left: 0; background: radial-gradient(circle, rgba(1,27,10,0.9) 0%, rgba(0,2,1,0.98) 100%); 
        }
        .header-container { width: 100%; max-width: 350px; display: flex; flex-direction: column; gap: 4px; align-items: center; }
        .trust-banner { font-size: 8px; font-weight: 900; color: #fff; background: rgba(0,50,25,0.9); padding: 3px 6px; border-radius: 5px; border: 1px solid #ffd700; width: 100%; }
        .royal-main-title { font-size: 18px; font-weight: 900; color: #ffd700; text-shadow: 0 0 15px #ffd700; letter-spacing: 1px; }
        
        .login-card { background: rgba(2, 43, 16, 0.95); border: 2px solid #ffd700; padding: 12px; border-radius: 14px; width: 92%; max-width: 280px; box-shadow: 0 10px 30px rgba(0,0,0,0.9); }
        .login-card input { width: 100%; padding: 7px; margin: 3px 0; background: #000; border: 1.5px solid #ffd700; color: #ffd700; border-radius: 5px; font-size: 11px; text-align: center; font-weight: bold; }
        .login-btn { width: 100%; padding: 8px; margin-top: 4px; background: linear-gradient(135deg, #ffd700, #b8860b); color: #000; border: none; font-weight: 900; border-radius: 5px; cursor: pointer; font-size: 10.5px; text-transform: uppercase; }
        .login-btn.lobby { background: linear-gradient(135deg, #27ae60, #1e8449); color: #fff; }

        /* 🟢 RESPONSIVE GAME LOBBY */
        #gameLobbyScreen { 
        ...
        .lobby-header { display: flex; justify-content: space-between; align-items: center; background: rgba(0, 26, 8, 0.95); border: 1.5px solid #ffd700; padding: 3px 8px; border-radius: 6px; width: 100%; max-width: 420px; flex-shrink: 0; }
        .user-profile-widget { display: flex; align-items: center; gap: 4px; background: rgba(0,20,8,0.9); border: 1px solid #ffd700; padding: 2px 6px; border-radius: 8px; cursor: pointer; }
        .user-avatar-circle { width: 24px; height: 24px; border-radius: 50%; background: #ffd700; display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: bold; color: #000; }
        
        .wallet-btn-rect { background: linear-gradient(135deg, #27ae60, #1e8449); color: #fff; border: 1px solid #ffd700; padding: 5px 10px; border-radius: 5px; font-size: 9.5px; font-weight: 900; cursor: pointer; text-transform: uppercase; flex-shrink: 0; }
        ...
        .timer-strip-center { 
            ...
            color: #fff; padding: 2px 8px; border-radius: 5px; border: 1px solid #ffd700; 
            width: 75px; text-align: center; display: flex; flex-direction: column; justify-content: center; align-items: center; flex-shrink: 0;
        }
        .timer-main-val { font-size: 13px; font-weight: 900; line-height: 1.1; }
        .timer-sub-val { font-size: 5px; color: #ffcccc; font-weight: bold; }

        .history-strip-bar { display: flex; justify-content: center; align-items: center; gap: 3px; background: rgba(0, 15, 5, 0.95); border: 1px solid #ffd700; padding: 2px 6px; border-radius: 5px; width: 100%; max-width: 420px; flex-shrink: 0; }
        .history-title { font-size: 8px; font-weight: 900; color: #ffd700; margin-right: 3px; }
        .history-ball { width: 17px; height: 17px; border-radius: 50%; font-size: 9px; font-weight: 900; color: #fff; display: flex; align-items: center; justify-content: center; border: 1px solid #ffd700; flex-shrink: 0; }
        ...
        .lobby-wheel-box { 
            position: relative; 
            width: min(45vh, 210px); 
            height: min(45vh, 210px); 
            margin: 2px auto; 
        ...
        #lobbyWheelCanvas { width: 100% !important; height: 100% !important; object-fit: contain; border-radius: 50%; display: block; border: 2px solid #ffd700; }
        ...
        .wheel-center-graphic {
            ...
            border-radius: 50%; border: 2px solid #ffd700; z-index: 110;
            display: flex; flex-direction: column; justify-content: center; align-items: center; pointer-events: none;
        }
        .hub-text-royal, .hub-text-digital { font-size: 6px; font-weight: 900; color: #ffd700; line-height: 1; }
        ...
        .game-ball-orbit::after { content: ""; position: absolute; top: 4px; left: calc(50% - 5px); width: 10px; height: 10px; background: radial-gradient(circle, #fff 0%, #00ffcc 60%, #ff0055 100%); border-radius: 50%; box-shadow: 0 0 10px #fff; }

        /* 🟢 COMPACT BETTING BOARD */
        .betting-board-section { background: rgba(1, 35, 12, 0.98); border-radius: 8px; padding: 3px 6px; width: 100%; max-width: 420px; display: flex; flex-direction: column; gap: 2px; border: 1.5px solid #ffd700; flex-shrink: 0; }
        .grid-table { display: grid; grid-template-columns: repeat(12, 1fr); gap: 1.5px; }
        
        .table-cell { background: linear-gradient(135deg, #042e12, #011506); border: 1px solid #d4af37; color: #e0e0e0; font-size: 10px; font-weight: 900; padding: 4px 0; border-radius: 3px; cursor: pointer; text-align: center; position: relative; }
        ...
        .table-cell.has-bet, .color-btn.has-bet, .group-btn.has-bet { border: 2px solid #fff !important; box-shadow: 0 0 10px #f1c40f; }
        .cell-badge { position: absolute; top: -4px; right: -3px; background: #f1c40f; color: #000; font-size: 6.5px; font-weight: 900; padding: 1px 2px; border-radius: 50%; z-index: 10; }

        .group-buttons-row-1 { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2px; width: 100%; }
        .group-buttons-row-2 { display: grid; grid-template-columns: repeat(2, 1fr); gap: 2px; width: 100%; }
        .group-btn { background: #07100b; border: 1px solid #d4af37; color: #ffd700; font-size: 8.5px; font-weight: 900; padding: 3px 2px; border-radius: 3px; cursor: pointer; text-transform: uppercase; }

        .color-row { display: flex; gap: 2px; width: 100%; }
        .color-btn { flex: 1; padding: 3px 2px; font-weight: 900; font-size: 9px; border-radius: 3px; border: 1px solid #ffd700; cursor: pointer; color: #fff; text-transform: uppercase; }
        .btn-red { background: #962d22; }
        .btn-green { background: #1e8449; }
        .btn-black { background: #1b2631; }

        .chip-selector-row { display: flex; justify-content: center; gap: 4px; align-items: center; width: 100%; }
        .chip-item { width: 22px; height: 22px; border-radius: 50%; border: 1.5px dashed #ffd700; font-weight: 900; font-size: 8.5px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: #fff; background: #111; }
        .chip-item.selected { border: 2px solid #fff; transform: scale(1.1); box-shadow: 0 0 10px #ffd700; }
        ...
        .transfer-btn-small { background: #2980b9; color: #fff; border: 1px solid #ffd700; padding: 3px 5px; border-radius: 3px; font-size: 7.5px; font-weight: bold; cursor: pointer; }
        .lobby-action-row { display: flex; justify-content: space-between; align-items: center; gap: 3px; width: 100%; }
        .status-msg-box { background: #f1c40f; color: #000; border: 1px solid #fff; padding: 2px 4px; font-size: 8px; font-weight: 900; border-radius: 4px; flex-grow: 1; text-align: center; }
        .clear-btn { background: #c0392b; color: #fff; padding: 3px 6px; font-size: 8px; font-weight: 900; border-radius: 3px; cursor: pointer; border: 1px solid #ffd700; }

        .mode-toggle-bar { display: flex; justify-content: center; gap: 2px; background: #000; border: 1px solid #ffd700; padding: 1px; border-radius: 4px; width: 100%; }
        .mode-btn { flex: 1; padding: 2px; font-size: 8px; font-weight: bold; border: none; border-radius: 2px; cursor: pointer; background: #111; color: #888; }
        .mode-btn.active-real { background: #27ae60; color: #fff; }
        .mode-btn.active-test { background: #2980b9; color: #fff; }

        .wallet-modal { display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.9); z-index: 30000; align-items: center; justify-content: center; padding: 10px; }
        .wallet-modal-content { background: radial-gradient(circle, #023814 0%, #000 100%); border: 3px solid #ffd700; padding: 15px; border-radius: 14px; width: 100%; max-width: 350px; text-align: center; color: #fff; max-height: 90vh; overflow-y: auto; }
        .withdraw-packages-grid, .deposit-packages-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 5px; margin: 6px 0; }
        .dep-pkg-btn, .wd-pkg-btn { background: #000; border: 1px solid #ffd700; color: #ffd700; padding: 7px 0; font-weight: bold; font-size: 10px; border-radius: 4px; cursor: pointer; }
        .dep-pkg-btn.selected, .wd-pkg-btn.selected { background: #ffd700; color: #000; }
        .wallet-modal-content input { width: 100%; padding: 8px; margin: 4px 0; background: #000; border: 1px solid #ffd700; color: #ffd700; border-radius: 5px; text-align: center; font-weight: bold; font-size: 11px; }

        #secretAdminModal { display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #020f06; z-index: 2147483647 !important; padding: 10px; text-align: center; color: #fff; overflow-y: auto; }
        .admin-dashboard-container { max-width: 800px; margin: 0 auto; background: #000; border: 2px solid #ffd700; border-radius: 12px; padding: 15px; display: flex; flex-direction: column; gap: 10px; text-align: left; }
        .admin-section-box { background: rgba(0,0,0,0.8); border: 1px solid rgba(255,215,0,0.3); padding: 10px; border-radius: 8px; font-size: 10.5px; }
        .req-table-row { background: #0b1c11; border: 1px solid #333; padding: 6px 10px; border-radius: 5px; margin-bottom: 4px; display: flex; justify-content: space-between; align-items: center; font-size: 10px; }
        .req-actions button { padding: 4px 8px; font-weight: bold; border-radius: 4px; border: none; cursor: pointer; font-size: 9px; }
    </style>
...
<body>
    ...
    <div id="customAlertModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.85); z-index:900000; align-items:center; justify-content:center; padding:15px;">
        <div style="background:#034519; border:3px solid #ffd700; padding:20px; border-radius:14px; width:100%; max-width:280px; text-align:center;">
            <h3 style="color:#ffd700; font-size:15px; margin-bottom:6px;">NOTICE</h3>
            <p id="customAlertMessage" style="font-size:11px; color:#fff; font-weight:bold; margin-bottom:12px;"></p>
            <button onclick="closeCustomAlert()" class="login-btn lobby" style="padding:8px; font-size:10px;">OK</button>
        </div>
    </div>
    ...
    <div id="adminPinModal" style="display:none; position:fixed; top:0; left:0; width:100vw; height:100vh; background:rgba(0,0,0,0.9); z-index:200000; align-items:center; justify-content:center; padding:15px;">
        <div style="background:#011c08; border:3px solid #ffd700; padding:18px; border-radius:14px; width:100%; max-width:260px; text-align:center;">
            <h3 style="color:#ffd700; font-size:14px; margin-bottom:6px;">ADMIN PIN</h3>
            <input type="password" id="adminSecretPassInput" placeholder="Enter PIN" maxlength="6" style="width:100%; padding:7px; background:#000; color:#ffd700; border:1px solid #ffd700; text-align:center; font-weight:bold; border-radius:4px; margin-bottom:8px;">
            <div style="display:flex; gap:5px;">
                <button onclick="verifyAdminPinCode()" class="login-btn lobby" style="padding:7px; font-size:10px;">Unlock</button>
                <button onclick="document.getElementById('adminPinModal').style.display='none'" style="background:#c0392b; color:#fff; border:none; padding:7px; border-radius:4px; font-size:10px; font-weight:bold; flex:1; cursor:pointer;">Cancel</button>
            </div>
        </div>
    </div>

    <div id="supportTicketModal" class="wallet-modal">
        <div class="wallet-modal-content" style="max-width:320px;">
            <h3 style="color:#ffd700; margin-bottom:6px; font-size:14px;">SUPPORT DESK</h3>
            <textarea id="supportUserMessageInput" placeholder="Describe your issue..." style="width:100%; height:80px; background:#000; color:#ffd700; border:1px solid #ffd700; padding:6px; border-radius:5px; font-size:10px; resize:none; margin-bottom:6px;"></textarea>
            <button onclick="submitSupportTicket()" class="login-btn lobby" style="padding:8px; font-size:10px;">Send Ticket</button>
            <button onclick="document.getElementById('supportTicketModal').style.display='none'" style="background:#444; color:#fff; border:none; padding:7px; width:100%; border-radius:5px; margin-top:5px; font-size:9.5px; font-weight:bold; cursor:pointer;">Close</button>
        </div>
    </div>
    ...
    <div id="comingSoonNoticeModal" style="display:flex; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:70000; align-items:center; justify-content:center;">
        <div style="background:#0b2d13; border:3px solid #ffd700; padding:18px; border-radius:16px; width:290px; text-align:center; color:#fff;">
            <h3 style="color:#ffd700; font-size:16px; margin-bottom:4px;">ROYAL CASINO</h3>
            <p style="font-size:10px; color:#fff; font-weight:bold; margin-bottom:6px;">100% SECURE & LIVE</p>
            <div style="background:#c0392b; color:#fff; font-size:10px; font-weight:bold; padding:5px; border-radius:4px; margin-bottom:6px; text-transform:uppercase;">🔥 7 PLAYER LIVE TABLE 🔥</div>
            <button onclick="closeComingSoonModal(); playMasterAudio();" style="background:linear-gradient(135deg, #ffd700, #b8860b); color:#000; border:none; padding:8px 20px; border-radius:6px; font-weight:900; font-size:11px; cursor:pointer; text-transform:uppercase;">Enter Lobby</button>
        </div>
    </div>

    <div id="fullscreenWheelBg">
        <div class="bg-wheel-wrap"><canvas id="loginWheelCanvasBg" width="320" height="320"></canvas></div>
    </div>
    ...
    <div id="loginScreenContainer">
        ...
        <div class="login-card">
            <h3 style="color:#ffd700; font-size:11px; margin-bottom:2px;">CLOUD LOGIN</h3>
        ...
        <div style="width:100%; max-width:300px;">
            <a onclick="openSupportModal();" style="display:inline-block; padding:3px 8px; background:#0088cc; color:#fff; text-decoration:none; font-weight:bold; border-radius:10px; font-size:7.5px; cursor:pointer;">🎧 Support Ticket</a>
        </div>
    </div>
    ...
    <div id="gameLobbyScreen">
        <div class="lobby-header">
            <div class="user-profile-widget" onclick="handleProfileClick(); event.stopPropagation();">
                ...
                <div>
                    <div style="font-size:7px; font-weight:900; color:#ffd700;" id="userDisplayPhone">RD001001</div>
                    <div style="font-size:5.5px; color:#2ecc71;">Profile 👆</div>
                </div>
            </div>
        ...
        <div class="lobby-wheel-box">
            <canvas id="lobbyWheelCanvas" width="220" height="220"></canvas>
        ...
        <div class="betting-board-section">
            <div style="background: rgba(0,0,0,0.85); padding: 1px 4px; border-radius: 4px; display: flex; justify-content: space-between; align-items: center; font-size: 8px; font-weight: 900; color: #ffd700;">
    ...
    <div id="playerProfileModal" class="wallet-modal">
        <div class="wallet-modal-content">
            <h3 style="color:#ffd700; margin-bottom:4px; font-size:14px;">VIP PASSBOOK</h3>
            <div style="background:#000; border:1px solid #ffd700; padding:6px; border-radius:6px; text-align:left; font-size:9.5px; margin-bottom:5px;">
                <div>📱 Mobile: <span id="profPhone">--</span></div>
                <div>🆔 VIP ID: <span id="profVipId">--</span></div>
                <div>💎 Real Bal: ₹<span id="profRealBal">0</span></div>
            </div>
            <div id="adminDirectAccessBtnWrap" style="display:none; margin-bottom:5px;">
                <button onclick="document.getElementById('playerProfileModal').style.display='none'; openAdminPanelDirectly();" style="width:100%; padding:7px; background:#ffd700; color:#000; font-weight:900; border:none; border-radius:4px; font-size:10px;">👑 ADMIN PANEL</button>
            </div>
            <div style="background:rgba(0,20,8,0.95); border:1px solid #2ecc71; border-radius:6px; padding:6px; text-align:left; max-height:130px; overflow-y:auto;" id="playerLedgerListContainer">
                <div style="color:#888; text-align:center;">Loading passbook...</div>
            </div>
            <button onclick="closeWalletModals()" class="login-btn" style="padding:7px; font-size:10px; margin-top:5px;">Close</button>
        </div>
    </div>
    ...
    <div id="secretAdminModal">
        <div class="admin-dashboard-container">
            <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #ffd700; padding-bottom:8px;">
                <h2 style="font-size:14px; color:#ffd700;">ADMIN CONTROL PANEL</h2>
                <button onclick="document.getElementById('secretAdminModal').style.display='none'" style="background:#e74c3c; color:#fff; border:none; padding:4px 10px; border-radius:4px; font-size:10px;">✕ Close</button>
            </div>
            <div class="admin-section-box">
                <div style="color:#3498db; font-weight:bold; margin-bottom:4px;">🎧 SUPPORT TICKETS</div>
                <div id="adminSupportTicketsList" style="max-height:100px; overflow-y:auto;">No tickets.</div>
            </div>
            <div class="admin-section-box">
                <div style="color:#2ecc71; font-weight:bold; margin-bottom:4px;">🟢 LIVE PLAYERS</div>
                <div id="adminLiveOnlinePlayersList" style="max-height:80px; overflow-y:auto;">Waiting...</div>
            </div>
            <div class="admin-section-box">
                <div style="color:#ffd700; font-weight:bold; margin-bottom:4px;">⚙️ RIG WINNING NUMBER</div>
                <select id="adminRigSelect" style="width:100%; padding:6px; background:#000; color:#ffd700; border:1px solid #ffd700; font-size:10px; margin-bottom:5px;">
                    <option value="random">Auto 5% Safe Engine (Random)</option>
                </select>
                <button onclick="saveAdminSettings()" class="login-btn lobby" style="padding:7px; font-size:10px;">Push Rig Number</button>
            </div>
            <div class="admin-section-box">
                <div style="color:#ffd700; font-weight:bold; margin-bottom:4px;">📥 DEPOSITS</div>
                <div id="admDepositList" style="max-height:100px; overflow-y:auto;">No deposits.</div>
            </div>
            <div class="admin-section-box">
                <div style="color:#ffd700; font-weight:bold; margin-bottom:4px;">📤 WITHDRAWALS</div>
                <div id="admWithdrawList" style="max-height:100px; overflow-y:auto;">No withdrawals.</div>
            </div>
        </div>
    </div>
    ...
    <div id="depositModalBox" class="wallet-modal">
        <div class="wallet-modal-content">
            <h3 style="color:#ffd700; font-size:14px; margin-bottom:6px;">DEPOSIT GATEWAY</h3>
            <input type="number" id="customDepositInput" value="300" placeholder="Amount">
            <img id="dynamicPlayerQrImg" src="https://i.ibb.co/3yk54L2/1000532596.jpg" style="width:120px; height:120px; object-fit:contain; margin:5px 0; border:2px solid #ffd700; background:#fff;" />
            ...
            <button onclick="document.getElementById('depositProofFileInput').click()" style="width:100%; padding:6px; background:#2980b9; color:#fff; border:none; border-radius:4px; font-size:9.5px; margin-bottom:5px; font-weight:bold;">📸 Upload Screenshot</button>
            <button onclick="triggerCloudDepositSubmit();" class="login-btn lobby" style="padding:8px; font-size:10px;">Submit Deposit</button>
            <button onclick="closeWalletModals()" style="background:#333; color:#ccc; border:none; padding:6px; width:100%; border-radius:4px; margin-top:5px; font-size:9.5px;">Close</button>
        </div>
    </div>

    <div id="withdrawModalBox" class="wallet-modal">
        <div class="wallet-modal-content">
            <h3 style="color:#ffd700; font-size:14px; margin-bottom:6px;">WITHDRAWAL GATEWAY</h3>
            ...
            <button onclick="triggerCloudSubmitWithdraw()" class="login-btn" style="background:#e74c3c; padding:8px; font-size:10px; margin-top:5px;">Request Payout</button>
            <button onclick="closeWalletModals()" style="background:#333; color:#ccc; border:none; padding:6px; width:100%; border-radius:4px; margin-top:5px; font-size:9.5px;">Close</button>
        </div>
    </div>
    ...
    <div id="transferModalBox" class="wallet-modal">
        <div class="wallet-modal-content">
            <h3 style="color:#ffd700; font-size:14px; margin-bottom:6px;">TRANSFER COINS</h3>
            ...
            <button onclick="executeCoinTransferCloud()" class="login-btn" style="background:#2980b9; padding:8px; font-size:10px; margin-top:5px;">Transfer</button>
            <button onclick="closeWalletModals()" style="background:#444; color:#fff; border:none; padding:6px; width:100%; border-radius:4px; margin-top:5px; font-size:9.5px;">Close</button>
        </div>
    </div>
    ...
    <div id="cinematicResultPopup" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.85); z-index:50000; flex-direction:column; align-items:center; justify-content:center;">
        <div style="background:#034519; border:3px solid #f1c40f; padding:18px; border-radius:16px; width:260px; text-align:center;">
            <h3 style="color:#f1c40f; font-size:14px; margin-bottom:4px;">WINNING NUMBER</h3>
            <div id="popupWinningNumDisplay" style="font-size:36px; font-weight:900; color:#fff; margin:8px 0;">14 RED</div>
            <div id="popupPayoutDisplay" style="font-size:11px; font-weight:bold; color:#00ffff;">You Won!</div>
        </div>
    </div>
    ...
    <script>
        ...
        let activeRoundId = 0, processedResultRoundId = null;
        ...
        window.addEventListener('DOMContentLoaded', () => {
            drawCanvasWheel('loginWheelCanvasBg', 55, 14);
            drawCanvasWheel('lobbyWheelCanvas', 45, 11);
        ...
        supportTicketsRef.on("value", (snap) => {
            ...
            Object.keys(tickets).forEach(k => {
                let t = tickets[k];
                if (t.status === 'Pending') {
                    html += \`<div style="background:#000; padding:5px; border-radius:4px; margin-bottom:3px; font-size:9.5px;">
                        <b>\${t.phone}</b>: "\${t.message}"
                        <button onclick="supportTicketsRef.child('\${k}').update({status:'Resolved'})" style="background:#27ae60; color:#fff; border:none; padding:2px 5px; margin-left:5px; border-radius:3px; cursor:pointer;">Resolve</button>
                    </div>\`;
                }
            });
        ...
        livePlayersRef.on("value", (snap) => {
            ...
            Object.values(players).forEach(p => {
                if (now - p.lastSeen < 4000) {
                    html += \`<div style="font-size:9.5px; color:#2ecc71;">📱 \${p.phone} (\${p.vipId}) - ₹\${p.balance}</div>\`;
                }
            });
        ...
        function loadPlayerPassbook(phone) {
            ...
            usersRef.child(phone + "/ledger").limitToLast(20).on("value", (snapshot) => {
                ...
                Object.values(ledger).reverse().forEach(item => {
                    ...
                    html += \`<div style="font-size:9px; border-bottom:1px dashed #333; padding:2px 0;"><b>\${item.description || item.sourceType}</b>: <span style="color:\${col};">\${item.amount > 0 ? '+' : ''}₹\${item.amount}</span></div>\`;
                });
